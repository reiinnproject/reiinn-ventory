...
├── app.html                # Main app shell (sidebar + content area)
├── public/
│   ├── logo.png
│   └── views/              # HTML fragments per feature
│       ├── dashboard.html
│       ├── inventory.html
│       ├── scheduling.html
│       ├── deliveries.html
...
│       └── deployment.html
├── src/
│   ├── main.js             # Entry for index.html (login logic)
│   ├── app.js              # Entry for app.html (router, init)
│   ├── router.js           # Hash-based routing, loads view HTML
│   ├── api.js              # fetch wrapper for API calls
│   ├── auth.js             # Token storage, auth check
│   ├── modules/            # Feature-specific JS (one per view)
│   │   ├── dashboard.js
│   │   ├── inventory.js
│   │   ├── scheduling.js
│   │   ├── deliveries.js
...
- `api.js`: `api.get(url)`, `api.post(url, body)`, etc. — adds `Authorization: Bearer <token>` header
- `auth.js`: `getToken()`, `setToken()`, `logout()`, `isAuthenticated()`
- Each `modules/*.js` exports `init()` and handles its view’s DOM and API calls
...
- Each module’s `init()` fetches data from API and renders; form submits call `api.post()` / `api.put()`
...
The first deploy may fail until the API and MongoDB are wired; that’s expected. Once Phase 4–5 are complete, production will work.